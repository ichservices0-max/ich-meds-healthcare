...
function QueueTrackerContent() {
  ...
  const id = (params?.id as string) || searchParams?.get('id') || searchParams?.get('appointmentId')
  ...
  if (!appointment) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-ink-400 text-sm mt-1">Please select an upcoming appointment to track live queue position.</p>
        <Link href="/dashboard/appointments" className="btn-primary mt-4 inline-block">Go to Appointments</Link>
      </div>
    )
  }
      ...
      {patient && id && (
        <div className="mt-8 h-[500px]">
          <ChatBox 
            ...
            apiToken={typeof window !== 'undefined' ? localStorage.getItem('token') || '' : ''} 
          />
        </div>
      )}
    </div>
